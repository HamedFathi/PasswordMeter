...
export class PasswordMeter {
  ...
  private getRequirementsScore(text: string, ignoreCase: boolean): string[] {
    ...
    if (req) {
      let minLengthMsg =
        "The minimum password length is " + req.minLength + ".";
      let maxLengthMsg =
        "The maximum password length is " + req.maxLength + ".";
      let uppercaseLettersMinLengthMsg =
        "You must use at least " +
        req.uppercaseLettersMinLength +
        " uppercase letter(s).";
      let lowercaseLettersMinLengthMsg =
        "You must use at least " +
        req.lowercaseLettersMinLength +
        " lowercase letter(s).";
      let numbersMinLengthMsg =
        "You must use at least " + req.numbersMinLength + " number(s).";
      let symbolsMinLengthMsg =
        "You must use at least " + req.symbolsMinLength + " symbol(s).";
      ...
      if (req.uniqueLettersMinLength) {
        ...
        let isValid = Array.from(new Set(text.split(""))).length >= val;
